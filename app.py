...
def seleccion():
    ...
    if request.method == 'POST':
        ...
        query = database.session.query(RegistrosSyP).outerjoin(Education).outerjoin(Course).outerjoin(Experience).outerjoin(Language)
        
        # Aplicar los filtros dinámicamente
        filters = []
        if carrera_f != "Todas":
            filters.append(RegistrosSyP.career == carrera_f)
        if tipo_f != "Todas":
            filters.append(RegistrosSyP.user_type == tipo_f)
        if nivel != "Todas":
            filters.append(Education.level_education == nivel)
        if area != "Todas":
            filters.append(Course.area == area)
        if areat != "Todas":
            filters.append(Experience.area_e == areat)
        if idioma != "Todas":
            filters.append(Language.language == idioma)
        ...
        print("Número de resultados:", len(us))
