...
class RegistrosSyP(database.Model, UserMixin):
    ...
    def __init__(self, firstname, lastname, username, email, password, career, rol, photo,):
...
class Proyectos(database.Model):
    ...
    id = database.Column(database.Integer, primary_key=True,
                     unique=True, autoincrement=True)
    user_id = database.Column(database.Integer, database.ForeignKey('users.id'))
    categoria = database.Column(database.Enum('Estudiante', 'Egresado', 'Profesional'),nullable=True)
    name_proyecto = database.Column(database.String(250),nullable=True)
    carrera = database.Column(database.Enum('Ingeniería de Minas', 'Ingeniería en Geología', 'Ingeniería Ambiental', 'Ingeniería en Petróleos'),nullable=True)
    description = database.Column(database.String(250),nullable=True)
    start_date = database.Column(database.Date,nullable=True)
    end_date = database.Column(database.Date,nullable=True)
